...
namespace gf
{
    ...
    double Function::ssqm(TH1D *h, TF1 *f)
    ...
    void bin_smearing(int M, int nbins)
    {
        ...
        for (int i = 0; i < nbins; ++i)
        {
            ...
            double sum_sq = std::accumulate(bin_values[i].begin(), bin_values[i].end(), 0.0,
                                            [mean, i](double acc, double value)
    ...
    void fit_distribution()
    {
        ...
        TF1 f_fixed("f_fixed", "pow(cos([0]*x + [1]),2) + [2]", x_min, x_max);
        f_fixed.SetParameters(5.2, 1.8, 0.2);
        h->Fit("f_fixed", "R");

        std::vector<double> x,y,err,res;
        // Raccogliamo i valori dei bin (bin validi: 1..nbins)
        for(int i = 1; i <= nbins; ++i){
            double xc = h->GetBinCenter(i);
            double yh = h->GetBinContent(i);
            double yf = f_fixed.Eval(xc);
            double e = h->GetBinError(i);
            if (e == 0) e = 1;
            x.push_back(xc);
            y.push_back(yh);
            err.push_back(e);
            res.push_back(yh - yf); // residuo: data - fit
        }
        

        static TCanvas *cfit = new TCanvas("c_fit", "Fit e residui", 900, 600);
        // -- istogramma con funzione fittata --
        cfit->Divide(1, 2);
        cfit->cd(1);
        h->SetLineColor(kBlue);
        h->SetMarkerStyle(20);
        h->Draw("E");
        f_fixed.SetLineColor(kRed);
        f_fixed.Draw("SAME");

        // -- grafico dei residui --
        cfit->cd(2);
        TGraphErrors *gRes = new TGraphErrors(nbins, x.data(), res.data(), nullptr, err.data());
        gRes->SetTitle("Residui: y_{data} - y_{fit}, x , Residuo");
        ...
        for (auto &r : res)
            ssq += r * r;
        double dev = sqrt(ssq / nbins);
        std::cout << "Deviazione standard dei residui" << dev << std::endl;

        // -- Analisi sensibilità dei parametri --
        for (double dk : {-0.2, 0.0, 0.2})
        {
            for (double dphi : {-0.2, 0.0, 0.2})
            {
                for (double db : {-0.05, 0.0, 0.05})
                {
                    TF1 f_test("f_test", "pow(cos([0]*x + [1]),2) + [2]", x_min, x_max);
                    f_test.SetParameters(5.2 + dk, 1.8 + dphi, 0.2 + db);
                    double dev_test = myfun.ssqm(h, &f_test);
                    std::cout << "k = " << 5.2 + dk
                              << " phi = " << 1.8 + dphi
                              << " b = " << 0.2 + db
                              << " dev = " << dev_test << std::endl;
                }
            }
        }
        cfit->Update();
